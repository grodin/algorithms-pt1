...
public class Point implements Comparable<Point> {
  ...
  private static final Comparator<Point> POINT_COMPARATOR_Y_THEN_X =
      Comparator.comparing((Point pt) -> pt.y).thenComparing((Point pt) -> pt.x);
  ...
  public double slopeTo(Point that) {
    ...
    if (dx == 0) {
      ...
      final double slope = ((double) dy) / dx;
      if (isNegativeZero(slope)) {
        return 0.0;
      } else {
        return slope;
      }
  ...
  public int compareTo(Point that) {
    return POINT_COMPARATOR_Y_THEN_X.compare(this, that);
  }
  ...
  private static final long negativeZeroRawBits =
      Double.doubleToRawLongBits(-0.0);

  private boolean isNegativeZero(double value) {
    return Double.doubleToRawLongBits(value) == negativeZeroRawBits;
  }
